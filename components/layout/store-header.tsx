    ...
    <header className="sticky top-0 z-30 border-b border-black/8 bg-white/95 backdrop-blur">
      ...
      >
        <div className="flex h-20 items-center justify-between border-b border-black/8 px-5">
          <Logo />
          <button className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-black/10" onClick={() => setMobileOpen(false)} aria-label="Close menu">
            <X className="h-5 w-5" />
          </button>
        </div>
        <div className="space-y-6 px-5 py-6">
          ...
          <form onSubmit={submitSearch} className="flex items-center gap-2 rounded-full border border-black/10 bg-white px-4">
            <Input
              value={query}
              onChange={(event) => setQuery(event.target.value)}
              placeholder="Search product"
          ...
          <nav className="grid gap-2 text-lg font-semibold text-[#141414]">
            {navItems.map((item) => (
              <Link key={item.href} href={item.href} onClick={() => setMobileOpen(false)} className="rounded-xl px-4 py-3 hover:bg-white">
                {item.label}
              </Link>
            ))}
          </nav>
